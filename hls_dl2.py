...
# Key bands to download
key_band_names = ["B03", "B04", "B08", "B10", "B11", "Fmask"]

def extract_band(filename):
    """Extract band name from HLS filename like HLS.L30.T16TDS.2015013T163332.v2.0.B04.tif"""
        ...
        for e in entries:
            ...
            for link in e.get("links", []):
                ...
                if href.endswith(".tif") and "data#" in link.get("rel", ""):
                    band_name = extract_band(href.split("/")[-1])
                    if band_name in key_band_names:
                        band_links[band_name] = href
            if band_links:
                all_granules.append({"title": title, "date": date_str, "tile": tile, "band_links": band_links})

print("\nGranules with key bands: " + str(len(all_granules)))
...
# Pick latest per month Mar-Dec 2015-2016
selected = []
for year in ["2015", "2016"]:
    for month in range(3, 13):
        ...
        if ym in by_month:
            ...
            tiles = " ".join(sorted(set(g["tile"] for g in month_latest)))
            print("  " + ym + ": " + str(len(month_latest)) + " tiles from " + latest_date + " " + tiles)

print("\nSelected " + str(len(selected)) + " granules")
...
for i, g in enumerate(selected):
    ...
    print("\n[" + str(i+1) + "/" + str(len(selected)) + "] " + g["title"] + " " + g["date"] + " " + g["tile"])
    
    for band_name, url in g["band_links"].items():
        dest = gdir / (g["title"] + "." + band_name + ".tif")
        if dest.exists() and dest.stat().st_size > 0:
            print("  [skip] " + band_name)
            continue
        print("  " + band_name + "...", end=" ", flush=True)
        try:
